...
let newestYear = 0;
for (let i=0; i<catalog.length; i++) {
  let currentBookYear = catalog[i].year;
  if (currentBookYear == "Unknown") {
    continue;
  } else {
    if (currentBookYear < oldestYear) oldestYear = currentBookYear;
    if (currentBookYear > newestYear) newestYear = currentBookYear;
  }
}
